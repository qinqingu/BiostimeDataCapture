...
{
    public class FaReturnDocJsonService : AbstractJsonService
    {
        public string GetEgJqGridJson(IList<JieYueDocDto> list, long totalRecords, PagingParameter paging, int userId)
        {
        ...
        private string GetintervalDays(int jieyueTianshu, DateTime jieyueShijian, DateTime? zidingyiGuihuanShijian)
        {
            DateTime? guihuanshijian = GetGuihuanShijian(jieyueTianshu, jieyueShijian, zidingyiGuihuanShijian);
            if (guihuanshijian != null)
            {
                DateTime gDateTime = Convert.ToDateTime(guihuanshijian);
                int interval = new TimeSpan(DateTime.Now.Ticks - gDateTime.Ticks).Days;
                if (interval > 0)
                {
                    return "<span style='color:red'>" + interval + "天</span>";
                }
            }
            return "";
        }
